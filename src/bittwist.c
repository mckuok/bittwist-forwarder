...
void ethernet_header(unsigned char *buffer, int buflen, const struct resolved_packet_config *resolved_config)
{
...
void ip_header(unsigned char *buffer, int buflen, const struct resolved_packet_config *resolved_config)
{
...
void tcp_header(unsigned char *buffer, int buflen, const struct resolved_packet_config *resolved_config)
{
    ethernet_header(buffer, buflen, resolved_config);
...
void udp_header(unsigned char *buffer, int buflen, const struct resolved_packet_config *resolved_config)
{
...
void data_process(unsigned char *buffer, int buflen, const struct resolved_packet_config *resolved_config)
{
    struct iphdr *ip = (struct iphdr *)(buffer + sizeof(struct ethhdr));
    ++total;
    ...
    case 6:
        ++tcp;
        tcp_header(buffer, buflen, resolved_config);
        break;

    case 17:
        ++udp;
        udp_header(buffer, buflen, resolved_config);
        break;
...
int main(int argc, char **argv)
{
    ...
    /* process options */
    while ((c = getopt(argc, argv, "dvi:s:l:c:m:r:p:w:h")) != -1)
    {
        switch (c)
        {
        case 'd':
        ...
        case 'p':
            ...
            break;
        case 'w':
            config_file_length = strlen(optarg);
            config_file = malloc(sizeof(char) * (config_file_length + 1));
    ...
    if (loop > 0)
    {
        while (loop--)
        {
            for (i = optind; i < argc; i++) /* for each trace file */
                send_packets(device, argv[i], configs, config_count);
        }
    }
...
void send_packets(char *device, char *trace_file, struct resolved_packet_config *configs, int config_count)
{
...
void info(void)
{
    ...
    notice("TCP: %d  UDP: %d  Other: %d  Toatl: %d  \r", tcp, udp, other, total);
}
...
void ts_print(register const struct timeval *tvp)
{
    ...
    (void)printf("Packet sent at: %02d:%02d:%02d.%06u\n",
